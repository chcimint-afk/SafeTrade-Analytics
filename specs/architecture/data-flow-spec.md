...
* **Обработка:**
  1. Сервер сверяет текущую цену с уровнем Stealth SL. При достижении 3.0% (или TP 4.0% для тренда) позиция закрывается.
  2. Клиент отправляет POST-запрос на `/api/trading/record-trade`. Цены входа и выхода рассчитываются динамически от котировки и изменения баланса, исключая хардкод цен.
* **Результат:** Сделка зафиксирована в Supabase. Баланс обновлен.

### Шаг 6: Фиксация лимита дневных потерь (Circuit Breaker Trigger)
* **Вход:** POST-запрос записи сделки возвращает отрицательный результат.
* **Обработка:** Если сумма убытков за сутки превысила 1.0% от баланса, эндпоинт `/api/trading/circuit-breaker` активирует дневную блокировку.
